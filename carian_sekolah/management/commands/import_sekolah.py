...
class Command(BaseCommand):
    help = (
        "Import data sekolah dari fail CSV dan hasilkan laporan ralat yang terperinci."
    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            with open(csv_file_path, mode="r", encoding="utf-8") as file:
                ...
                for row_num, row in enumerate(reader, 2):  # Mula dari baris 2
                    try:
                        ...
                        if None in row:
                            raise ValueError(
                                f"Baris mempunyai lebih banyak lajur daripada pengepala. Data tambahan: {row[None]}"
                            )
                        ...
                        if not kod_sekolah or not latitude or not longitude:
                            raise ValueError(
                                "Data penting (kod_sekolah, latitude, atau longitude) kosong."
                            )
                        ...
                        data_sekolah = {
                            ...
                            "school_type": row.get("school_type", "RENDAH")
                            .strip()
                            .upper(),
                        ...
                        _, dicipta = School.objects.update_or_create(
                            kod_sekolah=kod_sekolah, defaults=data_sekolah
                        )
                        success_count += 1
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Proses selesai. {success_count} sekolah berjaya diimport/dikemas kini."
                )
            )
            ...
            if error_rows:
                self.stdout.write(
                    self.style.WARNING(
                        f"{len(error_rows)} baris gagal diimport. Laporan ralat disimpan di {error_file_path}"
                    )
                )

                with open(
                    error_file_path, mode="w", encoding="utf-8", newline=""
                ) as error_file:
                    # Guna 'extrasaction' untuk mengabaikan lajur yang tidak dijangka
                    writer = csv.DictWriter(
                        error_file, fieldnames=error_fieldnames, extrasaction="ignore"
                    )
                    writer.writeheader()
