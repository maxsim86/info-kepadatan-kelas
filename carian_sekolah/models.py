...
class School(models.Model):
    ...
    kod_sekolah = models.CharField(max_length=10, unique=True, verbose_name="Kod Sekolah", null=True, blank=True)
    ...
    postcode = models.CharField(max_length=10, verbose_name="Poskod", blank=True, null=True)
    school_type = models.CharField(max_length=10, choices=SchoolType.choices, default=SchoolType.RENDAH, verbose_name="Jenis Sekolah")
    
    # === DIPERBAIKI: Hanya SATU medan 'location' ===
...
class SchoolImageSubmission(models.Model):
    class StatusChoices(models.TextChoices): # Nama kelas diubah untuk konsistensi
        PENDING = 'PENDING', 'Menunggu Kelulusan'
        APPROVED = 'APPROVED', 'Diluluskan' # Guna APPROVED untuk sepadan dengan admin
        REJECTED = 'REJECTED', 'Ditolak'
        
    school = models.ForeignKey(School, on_delete=models.CASCADE, related_name='submissions', verbose_name='sekolah')
    image = models.ImageField(upload_to='submissions/', verbose_name='Gambar yang dimuat naik')
    status = models.CharField(max_length=10, choices=StatusChoices.choices, default=StatusChoices.PENDING, verbose_name='status')
    uploaded_at = models.DateTimeField(auto_now_add=True, verbose_name='Tarikh dimuat naik')
